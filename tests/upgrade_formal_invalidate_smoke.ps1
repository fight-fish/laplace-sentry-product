...
function New-Case {
 param([string]$Name,[switch]$RuledTarget,[switch]$RootMismatch,[switch]$TamperManifest)
 ...
 $id=Split-Path -Leaf $tx; [pscustomobject]@{schema=$FormalPrepareSchema;mode='PrepareFormal';state='prepared_pending_apply';result='prepared_pending_apply';transaction_id=$id;transaction_root=if($RootMismatch){Join-Path $root 'wrong'}else{$tx};transaction_parent=$root;target_commit=if($RuledTarget){$FormalUpgradeTargetCommit}else{'971ba498d613c2bb20d46e14855cc0b0a326602a'};manifests=[pscustomobject]$refs;formal_target_write_count=0;fixture_mode=$false;events=@('prepared');updated_at_utc=[DateTime]::UtcNow.ToString('o')}|ConvertTo-Json -Depth 12|Set-Content -LiteralPath (Join-Path $tx 'transaction-journal.json') -Encoding UTF8
...
try {
 ...
 Assert-True ($after.invalidation.reason_code -eq 'stale_target_commit' -and $after.invalidation.journal_sha256_before -eq $journalHash -and $after.invalidation.manifest_sha256.source -eq $sourceHash) 'success evidence is incomplete or inconsistent.'
